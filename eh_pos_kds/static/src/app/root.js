...
whenReady(() => {
    ...
    if (target) {
        const showLoadError = () => {
            target.innerHTML = `
                <div class="o_eh_kds_config_missing">
                    <p>Kitchen board could not load.</p>
                    <p class="o_eh_kds_config_hint">Please refresh the display after the latest update.</p>
                </div>`;
        };
        try {
            const mount = mountComponent(KdsBoard, target);
            if (mount && typeof mount.catch === "function") {
                mount.catch(showLoadError);
            }
        } catch {
            showLoadError();
        }
