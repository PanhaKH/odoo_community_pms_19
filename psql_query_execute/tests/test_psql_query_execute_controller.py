...
class FakeRequest:
    ...
    def __init__(self, report_model):
        ...
        user = type('User', (), {'has_group': lambda self, group: group == 'base.group_system'})()
        env = type('FakeEnv', (dict,), {})({'psql.query': report_model})
        env.user = user
        self.env = env
...
class TestXLSXReportController(TransactionCase):
    ...
    def test_get_report_xlsx_returns_xlsx_response(self):
        ...
        self.assertIn('Query_Report.xlsx', disposition)
