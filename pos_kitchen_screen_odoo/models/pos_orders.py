...
class PosOrder(models.Model):
    ...
    def _is_line_for_kitchen(self, line, kitchen_screen):
        product_categories = line.product_id.pos_categ_ids
        if not kitchen_screen.pos_categ_ids:
            # An unrestricted kitchen handles every POS line, including products
            # which have not been assigned a POS category yet.
            return True
        if not product_categories:
            return False
        return any(cat.id in kitchen_screen.pos_categ_ids.ids for cat in product_categories)

    def _get_status_from_kitchen_lines(self, kitchen_lines):
    ...
    def _sync_order_status_from_kitchen_lines(self, kitchen_screen):
        for order in self:
            kitchen_lines = order.lines.filtered(
                lambda line: order._is_line_for_kitchen(line, kitchen_screen)
            )
            if not kitchen_lines:
                continue
            new_status = order._get_status_from_kitchen_lines(kitchen_lines)
        ...
        for order in res:
            ...
            if kitchen_screen:
                ...
                for order_line in order.lines:
                    if order._is_line_for_kitchen(order_line, kitchen_screen):
        ...
        for order in self:
            ...
            if kitchen_screen:
                ...
                for line in order.lines:
                    if order._is_line_for_kitchen(line, kitchen_screen):
    ...
    def get_details(self, shop_id, *args, **kwargs):
        ...
        if "is_room_service_pending" in self.env["pos.order"]._fields:
            order_domain.append(("is_room_service_pending", "=", False))
        pos_orders = self.env["pos.order"].search(order_domain, order="date_order")
        pos_orders._sync_order_status_from_kitchen_lines(kitchen_screen)
        pos_lines = pos_orders.lines.filtered(
            lambda line: line.is_cooking and self._is_line_for_kitchen(line, kitchen_screen)
        )
    ...
    def action_pos_order_paid(self):
        ...
        if kitchen_screen:
            ...
            for order_line in self.lines:
                if self._is_line_for_kitchen(order_line, kitchen_screen):
                    order_line.write({
                        'is_cooking': True,
                        'order_status': 'draft',
                    })
    ...
    def order_progress_change(self):
        ...
        if kitchen_screen:
            for line in self.lines:
                if self._is_line_for_kitchen(line, kitchen_screen):
    ...
    def check_order(self, order_name):
        ...
        for line in pos_order.lines:
            if kitchen_screen.pos_categ_ids and line.product_id.pos_categ_ids and not any(
    ...
    def get_kitchen_orders(self, config_id):
        ...
        if 'is_room_service_pending' in self._fields:
            order_domain.append(('is_room_service_pending', '=', False))
        kitchen_orders = self.search(order_domain)
        orders_data = []
        for order in kitchen_orders:
            # Get only kitchen lines
            kitchen_lines = order.lines.filtered(lambda l: self._is_line_for_kitchen(l, kitchen_screen))
            if kitchen_lines:
                line_data = []
                for line in kitchen_lines:
                    line_data.append({
...
class PosOrderLine(models.Model):
    ...
    def order_progress_change(self):
        ...
        if self.order_status == 'ready':
            ...
            self.order_status = 'ready'
        order_status = self._sync_parent_order_status_from_lines()
        ...
        if old_status != self.order_status:
            ...
            self.env["bus.bus"]._sendone(channel, "notification", message)
        return {
            'line_status': self.order_status,
            'order_status': order_status,
        }

    def _sync_parent_order_status_from_lines(self):
        self.ensure_one()
        order = self.order_id
        kitchen_screen = self.env["kitchen.screen"].search(
            [("pos_config_id", "=", order.config_id.id)], limit=1
        ...
        if kitchen_screen:
            ...
            kitchen_lines = order.lines.filtered(lambda line: line.is_cooking)
        if not kitchen_lines:
