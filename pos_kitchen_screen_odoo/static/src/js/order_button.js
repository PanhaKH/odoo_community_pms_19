...
patch(ActionpadWidget.prototype, {
    ...
    async submitOrder() {
        ...
        if (!this.uiState.clicked) {
            ...
            try {
                await self.orm.call("pos.order", "check_order_status", ["", this.currentOrder.pos_reference]).then(function(result){
                    if (result == false){
    ...
    hasQuantity(order) {
        if (!order) {
            ...
            return (
                order.lines.reduce((totalQty, line) => totalQty + line.getQuantity(), 0) > 0
            );
        }
    },
    ...
    get categoryCount() {
        const orderChanges = this.pos.getOrderChanges();
