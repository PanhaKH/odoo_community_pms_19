...
class KitchenScreenDashboard extends Component {
    setup() {
        ...
        this.ready_stage = () => this.setStage('ready');
        this.waiting_stage = () => this.setStage('waiting');
        this.draft_stage = () => this.setStage('draft');
    ...
    async loadOrders() {
        ...
        try {
            ...
            this.state.completed_count = this.state.ready_count;
    ...
    async startCountdown(orderId, timeString,config_id) {
        ...
        const configId = Array.isArray(config_id) ? config_id[0] : (config_id || this.currentShopId);
    ...
    getEventValue(eventOrId) {
        if (typeof eventOrId === "number" || typeof eventOrId === "string") {
            return Number(eventOrId);
        }
        return Number(eventOrId?.currentTarget?.value || eventOrId?.target?.value || 0);
    }

    async accept_order(e) {
        const orderId = this.getEventValue(e);
        if (!orderId) return;
        try {
    ...
    async accept_order_line(e) {
        ...
        const lineId = this.getEventValue(e);
        if (!lineId) return;
        try {
            const result = await this.orm.call("pos.order.line", "order_progress_change", [lineId]);

            const line = this.state.lines.find(l => l.id === lineId);
            if (line) {
                line.order_status = result?.line_status || (line.order_status === 'ready' ? 'waiting' : 'ready');
                const orderId = Array.isArray(line.order_id) ? line.order_id[0] : line.order_id;
                const order = this.state.order_details.find(o => o.id === orderId);
                if (order && result?.order_status) {
    ...
    orderLines(order) {
        const orderLineIds = new Set(order.lines || []);
        return this.state.lines.filter((line) => {
            const lineOrderId = Array.isArray(line.order_id) ? line.order_id[0] : line.order_id;
            return orderLineIds.has(line.id) || lineOrderId === order.id;
        });
    }
